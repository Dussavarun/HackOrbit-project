...
import { User, BookOpen, GitBranch, Brain, TrendingUp, Target, Mic } from 'lucide-react';
...
const navItems = [
  ...
  { id: 'insights', label: 'Industry Insights', icon: TrendingUp, path: '/insights' },
  { id: 'progress', label: 'Progress', icon: Target, path: '/progress' },
  { id: 'interview', label: 'Live Interview', icon: Mic, path: '/interview' }
];

const Header = ({ user, onProfileClick, isAuthenticated }) => {
  const location = useLocation();
  const navigate = useNavigate();

  // Handler for unauthenticated nav click
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center space-x-4">
            {isAuthenticated ? (
              <div className="hidden sm:flex items-center space-x-2 cursor-pointer" onClick={onProfileClick}>
